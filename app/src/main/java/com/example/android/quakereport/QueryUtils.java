...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject jsonObjectRoot = new JSONObject(SAMPLE_JSON_RESPONSE); // Obtiene el nodo raíz
            JSONArray jsonArrayFeatures = jsonObjectRoot.getJSONArray("features"); //Lee el arreglo de terremotos
//            double magnitude;
            String location, time, url;
            double magnitude;
//            long time;
            for (int x = 0; x < jsonArrayFeatures.length(); x++) {
                JSONObject jsonObjectCurrentEarthquake = jsonArrayFeatures.getJSONObject(x);
                JSONObject jsonObjectProperties = jsonObjectCurrentEarthquake.getJSONObject("properties");
...
//                magnitude = jsonObjectProperties.getString("mag");
                ...
                time = jsonObjectProperties.getString("time");
                url = jsonObjectProperties.getString("url");

//                long timeMilliseconds = jsonObjectProperties.getLong("time");
//                Date dateObject = new Date(timeMilliseconds);
//                SimpleDateFormat simpleDateFormat  =new SimpleDateFormat("MMM DD, yyyy");
//                SimpleDateFormat simpleTimeFormat = new SimpleDateFormat("h:mm a");

//                time = simpleDateFormat.format(dateObject); // + "\n" + simpleTimeFormat.format(dateObject);
